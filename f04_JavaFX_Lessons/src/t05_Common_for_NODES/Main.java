...
//		hbox.getChildren().addAll(btn1, btn2, btn3);
		 ...
		 hbox.setMaxSize(100,100)			- устанавливает максимальную высоту и ширину окна
		   Аналогичные методы get..() используются для получения соответствующей информации */
			...
			/* ЗАДАНИЯ РАСТОЯНИЯ МЕЖДУ NODES B PANE
		...
		Region spacer = new Region();
			/* Для растягивания используется  
		МЕТОД static void setHgrow(Node child, Priority priority) - задает growth behavior для данного сhild node.
 		Список Priority enumeration:
	- Priority.NEVER - показывает, что ширина node никогд не должна корректироваться для заполнения всего доспупного пространства
	pane. Это настройка по умолчанию, поэтому nodes не изменяются при изменение размера pane, содежащего их.
	 - Priority.ALWAYS  - показывает, что ширина должна всего автоматически регулироваться для pane.
	 Если у двух и более nodes такая настройка, то свободное пространство делится поровну между ними. 
	 - 	Priority.SOMETIMES  - показывает, что ширина node может регулироваться, если необхдимо заполнить pane.
	Однако, такая регулировка может быть только, если нет других nodes с установкой  ALWAYS.
	Метод является статичным, поэтому правильнее вызывать его для класса HBox, но не для объекта hbox, хотя  HBox.setMargin() = hbox.setMargin() */
		...
		HBox root = new HBox(10, vbox2, vbox3, vbox4, vbox5, vbox6);	root.setMinWidth(900);    root.setMinHeight(400); root.setPadding(new Insets(10));
