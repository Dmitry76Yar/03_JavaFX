...
/* VM arguments прописываем --add-modules=javafx.controls,javafx.fxml,javafx.web
...
//------------------------------------------------------------------------------------------------------------------------
		Hyperlink hyperlink2 = new Hyperlink("ССЫЛКА1");			   
		Hyperlink hyperlink3 = new Hyperlink("ССЫЛКА2", imf);
...
//------------------------------------------------------------------------------------------------------------------------
		...
		toggleButton1.setOnAction(event -> {
			if (toggleButton1.isSelected())		toggleButton1.setText("TGB1 ВКЛ");
			else toggleButton1.setText("TGB1 ВЫКЛ");
		});
		toggleButton2.setOnAction(event -> {
			if (toggleButton2.isSelected())		toggleButton2.setText("TGB2 ВКЛ");
			else toggleButton2.setText("TGB2 ВЫКЛ");
		});
...
//------------------------------------------------------------------------------------------------------------------------
		...
		btnOK.setOnAction(event -> {
			String msg = "";
		if (radioButton1.isSelected()) msg = "Выбрана radioButton1";
		if (radioButton2.isSelected()) msg = "Выбрана radioButton2";
		if (radioButton3.isSelected()) msg = "Выбрана radioButton3";
...
//------------------------------------------------------------------------------------------------------------------------
		...
		Button btn = new Button("ПОДСКАЗКА");
...
//----------------------------------------------------------------------------------------------------------------------------
		...
		Button btn50 = new Button("Меню при нажатии правой клавиши");
...
//---------------------------------------------------------------------------------------------------------------------------------/
		...
		choice2.setPromptText("COMBOBOX Напиши/выбери");
		...
		choice2.setOnShowing(new EventHandler<Event>() {
			...
			public void handle(Event event) {
				lbl4.setText("ComboBox готов к выбору");
...
//-----------------------------------------------------------------------------------------------------------------------------
	...
	btn10.setOnAction(new EventHandler<ActionEvent>() {
		...
		public void handle(ActionEvent event) {
			...
			Alert alert = new Alert(AlertType.INFORMATION, "Выбраны элементы листа с индексами " + 
					obsIndex.toString() + " и со значением " + obsValue.toString());
					alert.showAndWait();
				}
	});
...
//------------------------------------------------------------------------------------------------------------------------
		...
		Text text5 = new Text("     TEXTFLOW Автоматический пословный перенос на следующую строку");
...
//------------------------------------------------------------------------------------------------------------------------
		...
		editor.setHtmlText(
				"<html><head><title>Название</title></head><body><body contenteditable=\"true\">" + 
		...
		Button btn5 = new Button("Получить HTML текст");
...
//---------------------------------------------------------------------------------------------------------------------------
	...
	public TreeItem<String> makeShow(String nodeName, TreeItem<String> parent) {
		// В параметре принимает имя узла в String и его родителя класса TreeItem  
		// Создает node класса TreeItem, в конструктор принимает String nodeName
	TreeItem<String> node = new TreeItem<String>(nodeName);
		// Методом setExpanded() устанаваливаем для этого node true, чтобы он дальше мог иметь детей в дереве
	node.setExpanded(true);
		/* Методом getChildren() получаем список ObservableList - список детей этого же родителя и добавляем
		   в этот список данный узел   */
