package t04_Scene_graph_Navigation_Search;
...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {			
		...
		Scene scene = new Scene(grid, 600, 600);
			...
			/* ПОИСК УЗЛА ПО ИДЕНТИФИКАТОРУ через метод Node lookup (String selector)
			Данный метод содержится в классах Node и Parent (scene)
			В качестве параметра selector метод принимает CSS-селектор. 
			Чтобы найти по ID, нужно ввести в начало строки #+id.  	Чтобы найти по стилевому классу, нужно указать +.styleClass
			Метод возвращает ПЕРВЫЙ найденный узел или null, если нет такого узла  		 */
		Node nd =  scene.lookup("#btn2");
		System.out.println("Результат поиска по lookup() по id - " + nd.getClass().getName() + " id = " + nd.getId());
		Node nd1 =  scene.lookup(".button");
		System.out.println("Результат поиска по lookup() по styleClass - " + nd1.getClass().getName() + " id = " + nd1.getId());
			
			/* ПОИСК УЗЛА ПО ИДЕНТИФИКАТОРУ через метод Node lookupAll (String selector)
			 Данный метод содержится только в класс Node  
			 Важно, что данный метод ищет узлы в более низких уровнях (дальше от корня) + в узле, для которого был вызван метод  	 */
		Set<Node> list2 = grid.lookupAll(".button");
		System.out.println("Результат поиска по lookupAll() для grid  - " + list2);
			// Ниже кнопки 1-9 не будут найдены, т.к. они находятся в верхнем уровне относительно hbox, а будет
		// найден только кнопка btn10, т.к. она находится в узле hbox, для которого вызывался метод lookupAll  
		Set<Node> list3 = hbox.lookupAll(".button");
		System.out.println("Результат поиска по lookupAll() для hbox  - " + list3);
			...
			Так как мы знаем идентификатор кнопки, то мы можем легко получить ссылку на кнопку, которая была нажата 
								ТАКЖЕ ПОКАЗАНА НАВИГАЦИЯ ПО ГРАФУ СЦЕНЫ												*/
		scene.addEventHandler(ActionEvent.ACTION, event -> {
			if (event.getTarget() instanceof Button) {
				Button a = (Button)event.getTarget();
				System.out.println("Text - " + a.getText() + "  , ID - " + a.getId() + "  , " + a.getUserData());
